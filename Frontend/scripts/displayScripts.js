...
function handleSwapStockAndETF(e) {
    ...
    if (getIsETFActive() && !loadedA) {
        getSideBarData(displayStockList, "a");
        loadedA = true;
    }
    else displayStockList("", true);
}
...
//Function used to reload stock list by filter
function handleFilterStockList(e) {
    var sender = e.srcElement || e.target;
    var test = event.keyCode;
    if (event.keyCode == 13 || event.keyCode == undefined) {
        setCursor("wait");
        event.preventDefault();
        displayStockList(document.getElementById('searchBox').value);
    }
}
...
function handleUpdateStockList(e) {
    var sender = e.srcElement || e.target;
    setCursor("wait");
    var etf = getIsETFActive();
    getSideBarData(displayStockList, letters[(etf) ? etfIndex : stockIndex]);
    if (etf) etfIndex++;
    else stockIndex++;
    setCursor("default");
}
...
function loadMovingAverage(crossover) {
    var ticker = getActiveStockTicker();
    var displayValue = getDisplayValueFilter();
    if (crossover) setMovingAverageFilterValue(["1W", "3M"])
