class Portfolio {
    ...
    */
    ...
    getStocksByFilter(filter) {
        filter = filter.trim().toLowerCase();
        if (filter == "") return this.stocks;
        var filteredStocks = [];
        for (var i = 0; i < this.stocks.length; i++) {
            if (this.stocks[i].getName().toLowerCase().includes(filter)) {
                filteredStocks.push(this.stocks[i]);
            }
        }
        return filteredStocks;
    }
    getETFsByFilter(filter) {
        var filteredETFs = [];
        filter = filter.trim().toLowerCase();
        if (filter == "") return this.ETFs;
        for (var i = 0; i < this.ETFs.length; i++) {
            if (this.ETFs[i].getName().toLowerCase().includes(filter)) {
                filteredETFs.push(this.ETFs[i]);
            }
        }
        return filteredETFs;
    }
