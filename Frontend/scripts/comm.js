...
function requestData(dataString, ticker, operation, field, timePeriod, incremental, loadLetter) {
    var message = "";
...
function getSideBarData(_callback, loadLetter) {
    //Set up variables
    var packetIndex = 0;
    var dataRequest;
    if (getIsETFActive()) {
        dataRequest = requestData("GetETFLabels", "", "", "", "", false, loadLetter);
    }
    else {
        dataRequest = requestData("GetStockLabels", "", "", "", "", false, loadLetter);
    }
    //Listen for response
    var listener = {
        status: function (statusEvent) {
        ...
        message: function (msg) {
            if (msg.message.operation == "ReturnAssetLabels") {
                if (msg.message.requester == (myPortfolio.getPubNubClient() + 1)) {
                    if (getIsETFActive()) {
...
function getAssetDataByTicker(ticker, reload, _callback) {
    ...
    var incremental = (reload) ? false : updateIncrementally("Trend");
